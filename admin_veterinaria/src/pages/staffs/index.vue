        ...
        <VCard title="Staff">
            <VCardText class="d-flex flex-wrap gap-4">
                ...
                <div class="d-flex gap-x-4 align-center">
                    <VBtn color="primary" v-if="isPermission('register_staff')" prepend-icon="ri-add-line"
            ...
            <VDataTable :headers="headers" :items="data" :items-per-page="5" class="text-no-wrap">
                ...
                <template #item.action="{ item }">
                    <div class="d-flex gap-1">
                        <IconBtn size="small" v-if="isPermission('edit_staff')" @click="editItem(item)">
                            <VIcon icon="ri-pencil-line" />
                        </IconBtn>
                        <IconBtn size="small" v-if="isPermission('delete_staff')" @click="deleteItem(item)">
            ...
            <AddStaffDialog v-if="roles.length > 0" v-model:is-dialog-visible="isAddStaffDialogVisible" :roles="roles"
                @addUser="addUser" />
            <EditStaffDialog v-if="staff_selected" :userSelected="staff_selected" :roles="roles"
                v-model:is-dialog-visible="isEditStaffDialogVisible" @editUser="editUser" />
            <DeleteStaffDialog v-if="staff_selected_deleted" :userSelected="staff_selected_deleted"
                @deleteUser="deleteUser" v-model:is-dialog-visible="isDeleteStaffDialogVisible" />
        </VCard>
