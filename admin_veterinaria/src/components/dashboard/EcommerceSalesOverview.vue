  ...
  try {
    ...
    const resp = await $api('/kpi_report_general', {
      ...
      onResponseError({ response }) {
        ...
        error_exists.value = response?._data?.error ?? response?.error ?? true;
      }
    });

    console.log('raw resp from $api:', resp);

    let data = resp._data ?? resp.data ?? resp;

    report_general.value.n_pets = Number(data.n_pets ?? 0);
    report_general.value.n_service_record = Number(data.n_service_record ?? 0);
    report_general.value.net_income_total = Number(data.net_income_total ?? 0);
    report_general.value.net_income_total_before = Number(data.net_income_total_before ?? 0);
    report_general.value.variation_percentage = Number(data.variation_percentage ?? 0);
    ...
    console.error('reportGeneral error:', error);
  }
}
  ...
  <VCard>
    <VCardItem>
      ...
      <VCardSubtitle class="d-flex align-center gap-x-2">
        ...
        <div class="d-flex align-center text-success font-weight-medium"
          v-if="report_general.variation_percentage >= 0">
    ...
    <VCardText>
      <div class="d-flex justify-space-between flex-column flex-sm-row gap-4 flex-wrap">
        <div v-for="item in statistics" :key="item.title" class="d-flex align-center">
          <VAvatar :color="item.color" rounded variant="tonal" size="40" class="me-3">
