  ...
  try {
    const resp = await $api('/kpi_total_bruto', {
      ...
      onResponseError({ response }) {
        ...
        // error_exists could ser una ref global; si no existe, comentar o manejar
        if (typeof error_exists !== 'undefined') error_exists.value = response._data?.error;
      }
    });
    console.log(resp);
...
<template>
  <VCard>
    <VCardText>
      <div class="d-flex align-center justify-space-between">
        ...
        <div class="d-flex justify-center" v-if="income_bruto_selected.variation_percentage >= 0">
          ...
          <VIcon icon="ri-arrow-up-s-line" color="success" size="20" />
        </div>

        <div class="d-flex justify-center" v-else>
          <span class="text-error">{{ income_bruto_selected.variation_percentage }}%</span>
          <VIcon icon="ri-arrow-down-s-line" color="error" size="20" />
        </div>
      </div>
    ...
    <VCardText>
      <VRow no-gutters>
        <!-- PAGADO: ahora AZUL (primary) -->
        <VCol cols="5">
          <div class="d-flex align-center mb-2">
            <VAvatar rounded color="primary" variant="tonal" :size="24" class="me-2">
              <VIcon size="16" icon="ri-pie-chart-2-line" />
            </VAvatar>
        ...
        <VCol cols="2">
          <div class="d-flex flex-column justify-center align-center h-100">
            <VDivider vertical class="mx-auto mb-2" />
            <VAvatar size="28" variant="tonal" class="text-body-2">
              VS
            </VAvatar>
            <VDivider vertical class="mx-auto mt-2" />
          </div>
        </VCol>

        <!-- DEUDA: ahora AMARILLO (warning) -->
        <VCol cols="5" class="text-end">
          <div class="d-flex align-center justify-end mb-2">
            <span class="me-2">Deuda</span>

            <VAvatar rounded="sm" color="warning" variant="tonal" :size="24">
              <VIcon size="16" icon="ri-hand-coin-line" />
            </VAvatar>
          </div>
      ...
      <div class="mt-4">
        <!-- Barra: relleno = Pagado (AZUL / primary), fondo = Deuda (AMARILLO / warning) -->
        <VProgressLinear :model-value="income_bruto_selected.percentage_payments" color="primary" bg-color="warning"
          bg-opacity="1" :rounded-bar="false" height="8" rounded />
      </div>
    </VCardText>
  </VCard>
</template>
